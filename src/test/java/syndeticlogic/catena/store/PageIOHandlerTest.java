...
public class PageIOHandlerTest {
    ...
    void doScan() {
        ...
        for (int i = 0, offset = 0; i < pages; i++, offset += pageSize) {
            ...
            int amountScanned = pio.scan(actual, 0, pageSize, offset);
    ...
    public void testSinglePageFixedLengthUpdate() {
        ...
        pio.append(zero, 0, size);
        pio.append(one, 0, size);
        pio.append(two, 0, size);
        pio.append(three, 0, size);

        assertEquals(page.size(), page.limit());

        randomData = new byte[size];
        pio.update(randomData, 0, size, size, 0);
        pio.update(randomData, 0, size, size, 3 * size);

        byte[] actual = new byte[size];
        int amount = pio.scan(actual, 0, size, 0);
        assertEquals(size, amount);
        assertArrayEquals(randomData, actual);

        amount = pio.scan(actual, 0, size, size);
        assertEquals(size, amount);
        assertArrayEquals(one, actual);

        amount = pio.scan(actual, 0, size, 2 * size);
        assertEquals(size, amount);
        assertArrayEquals(two, actual);

        amount = pio.scan(actual, 0, size, 3 * size);
        assertEquals(size, amount);
        assertArrayEquals(randomData, actual);
    }
    ...
    public void testVariableLengthSinglePageExpandFirstElement() {
        ...
        pio.update(randomData, 0, size, newSize, 0);

        byte[] actual = new byte[newSize];
        int amount = pio.scan(actual, 0, newSize, 0);
        assertEquals(newSize, amount);
        assertArrayEquals(randomData, actual);

        actual = new byte[size];
        amount = pio.scan(actual, 0, size, newSize);
        assertEquals(size, amount);
        assertArrayEquals(one, actual);

        amount = pio.scan(actual, 0, size, newSize + size);
        assertEquals(size, amount);
        assertArrayEquals(two, actual);

        amount = pio.scan(actual, 0, size, newSize + 2 * size);
        assertEquals(size, amount);
    ...
    public void testVariableLengthSinglePageExpandLastElement() {
        ...
        randomData = new byte[newSize];
        pio.update(randomData, 0, size, newSize, 2 * size);
        ...
        amount = pio.scan(actual, 0, newSize, 2 * size);
        assertEquals(newSize, amount);
        assertArrayEquals(randomData, actual);
    ...
    public void testVariableLengthSinglePageExpandInteriorElement() {
        ...
        randomData = new byte[newSize];
        pio.update(randomData, 0, size, newSize, size);
        ...
        actual = new byte[newSize];
        amount = pio.scan(actual, 0, newSize, size);
        assertEquals(newSize, amount);
        assertArrayEquals(randomData, actual);
    ...
    public void testVariableLength2PageExpandLastElement() {
        ...
        randomData = new byte[newSize];
        pio.update(randomData, 0, size, newSize, 3 * size);

        assertEquals(page.size(), page.limit());

        byte[] actual = new byte[newSize];
        int amount = pio.scan(actual, 0, newSize, 3 * size);
        assertEquals(newSize, amount);
        assertArrayEquals(randomData, actual);

        actual = new byte[size];
        amount = pio.scan(actual, 0, size, 0);
        assertEquals(size, amount);
        assertArrayEquals(zero, actual);
    ...
    public void testVariableLength2PageExpandInteriorElement() {
        ...
        byte[] actual = new byte[newSize];
        int amount = pio.scan(actual, 0, newSize, 2 * size);
        assertEquals(newSize, amount);
        assertArrayEquals(randomData, actual);
        ...
        amount = pio.scan(actual, 0, size, 2 * size + newSize);
        assertEquals(size, amount);
        assertArrayEquals(three, actual);
    ...
    public void doUpdateTest(int max, int iterations, int randBound,
        ...
        for (int i = 0; i < iterations; i++) {
            ...
            int amount = pio.scan(actual, 0, actual.length, fileOffset);
        ...
        for (int i = 0; i < iterations; i++) {
            ...
            if (randValue < randThreshold) {
                ...
                pio.update(next, 0, oldLen, next.length, offset);
                offset += next.length;
                for (int j = index + 1; j <= index + 1; j++) {
                    ...
                    int amount = pio.scan(actual, 0, actual.length, offset);
                    assertEquals(actual.length, amount);
                    assertEquals(expected.length, amount);
