...
public class CodecTest {
	...
	@Test
	public void testEncodeDecodeByteBuffer() {
		
		ByteBuffer code = ByteBuffer.allocate(234);
		code.mark();
		assertEquals(Type.BOOLEAN.length(),c.encode(true,code));
		code.reset();
		assertEquals(true, c.decodeBoolean(code));
		
		code.mark();
		assertEquals(Type.TYPE.length(),c.encode(Type.BINARY,code));
		code.reset();
		assertEquals(Type.BINARY, c.decodeCodecType(code));

		code.mark();
		assertEquals(Type.BYTE.length(),c.encode((byte)0xff,code));
		code.reset();
		assertEquals((byte)0xff, c.decodeByte(code));

		code.mark();
		assertEquals(Type.CHAR.length(),c.encode('c',code));		
		code.reset();
		assertEquals('c', c.decodeChar(code));

		code.mark();
		assertEquals(Type.SHORT.length(),c.encode((short)255,code));
		code.reset();
		assertEquals((short)255, c.decodeShort(code));
		
		code.mark();
		assertEquals(Type.INTEGER.length(),c.encode(3255,code));
		code.reset();
		assertEquals(3255, c.decodeInteger(code));
		
		code.mark();
		assertEquals(Type.LONG.length(),c.encode(38888255333L,code));
		code.reset();
		assertEquals(38888255333L, c.decodeLong(code));

		code.mark();
		assertEquals(Type.FLOAT.length(),c.encode(3.14159f,code));
		code.reset();
		assertEquals(3.14159f, c.decodeFloat(code), 0.0);
		
		code.mark();
		assertEquals(Type.DOUBLE.length(),c.encode(3.14159D,code));
		code.reset();
		assertEquals(3.14159D, c.decodeDouble(code), 0.0);
		
		code.mark();
		String s = new String("James Percent");
		assertEquals(Type.STRING.length()+s.getBytes().length, c.encode(s, code));
		code.reset();
		assertEquals("James Percent", c.decodeString(code));
		
		code.mark();
		assertEquals(Type.BINARY.length()+s.getBytes().length, c.encode(s.getBytes(),
				code));
		code.reset();
		byte[] ret = c.decodeBinary(code);//;);
		assertEquals(s.getBytes().length, ret.length);
		for(int i = 0; i < s.getBytes().length; i++) {
			//System.out.println("original, encoded, decoded == "+(int)s.getBytes()[i]+","+
			//		code[34+i]+","+(int)ret[i]);
